...
class ApplicationControllerSpec extends PlaySpecification with Mockito {
  ...
  trait Context extends Scope {
    ...
    class FakeModule extends AbstractModule with ScalaModule {
      def configure() = {
        bind[Environment[UserController, CookieAuthenticator]].toInstance(env)
      }
    }
     ...
     */
    implicit val env: Environment[UserController, CookieAuthenticator] = new FakeEnvironment[UserController, CookieAuthenticator](Seq(identity.loginInfo -> identity))
